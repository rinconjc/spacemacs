...
- [[#key-bindings-conventions][Key bindings conventions]]
  ...
  - [[#version-and-environment-managers][Version and Environment managers]]
  - [[#tests][Tests]]
...
Version and environment managment modes are inherently globals, they need to
control the execution environment for all buffers. This is the reason why the
key bindings for version and environment managers like Python =virtualenv= or
Ruby =rbenv= are bound globally.

The prefix for environment management commands is ~SPC V~.

Each language has its own prefix, for instance Python environment managers
are under ~SPC V p~.

The prefixes can be long in order to keep the risk of key bindings collision
low but this is an acceptable trade off.
