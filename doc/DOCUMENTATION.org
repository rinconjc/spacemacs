...
- [[#concepts][Concepts]]
  ...
  - [[#leader-keys][Leader keys]]
    - [[#top-leader-key][Top leader key]]
    - [[#major-mode-leader-keys][Major mode leader keys]]
    - [[#major-mode-leader-key][Major Mode leader key]]
    - [[#project-type-leader-key][Project Type leader key]]
...
- [[#differences-between-vim-evil-and-spacemacs][Differences between Vim, Evil and Spacemacs]]
  - [[#search-char-with-f-f-t-and-t-and-repeart-search-char-with--and-][Search char with f, F, t and T and repeart search char with , and ;]]
...
- [[#commands][Commands]]
  ...
  - [[#searching][Searching]]
    - [[#searching-for-a-character-in-the-buffer][Searching for a character in the buffer]]
      - [[#improved-f-f-t-and-t-search-behavior][Improved f, F, t and T search behavior]]
      - [[#two-characters-search-with-s-and-s][Two-characters search with s and S]]
      - [[#available-search-scopes][Available search scopes]]
      - [[#characters-grouping][Characters grouping]]
...
** Leader keys
Spacemacs uses several leader keys to be able to bind thousands of key bindings:
- the top leader key ~SPC~
- the major mode leader keys ~,~ or ~SPC m~
- the project types leader keys ~;~ or ~SPC M~

*** Top leader key
The top leader key is ~SPC~ (the space bar, hence the name =Spacemacs=). This
key is the most accessible key on a keyboard and it is pressed with the thumb
which is a good choice to lower the risk of [[https://en.wikipedia.org/wiki/Repetitive_strain_injury][RSI]]. It can be customized to any
other key using the variable =dotspacemacs-leader-key= and
=dotspacemacs-emacs-leader-key=.

Under the ~SPC~ leader key we can find all the global commands accessible from
anywhere in Emacs as well as the to other leader keys for the current major mode
and project types.

Every command are executed very easily in =normal= mode by pressing the ~SPC~
leader key followed by a key sequence. Some examples:
- Save a buffer: ~SPC f s~
- Save all opened buffers: ~SPC f S~
- Open (switch) to a buffer: ~SPC b b~

A command key sequence following the leader key are most of the time a mnemonic
sequence which is easier to remember.

*** Major mode leader keys
Commands relative to the currently enabled [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Major-Modes.html][major mode]] are under the leader key
~,~. Internally ~,~ is a shortcut for the sequence ~SPC m~ (remember that all
Spacemacs commands start with ~SPC~. Pessing ~,~ or ~SPC m~ is therefore
identical.
SPC m~. For
 convenience a shortcut key called the major mode leader key is set by default
 on ~​,​~ which saves one keystroke.
...
*** Project Type leader key
Key bindings specific to the current =project type= start with ~SPC M~. For
convenience a shortcut key called the project type leader key is set by
default on ~;~ which saves one keystroke.

It is possible to change the major mode leader key by defining the variable
=dotspacemacs-project-minor-mode-leader-key= in your =~/.spacemacs=. For example
to setup the key on tabulation:

#+BEGIN_SRC emacs-lisp
  (setq-default dotspacemacs-project-minor-mode-leader-key "<tab>")
#+END_SRC
...
symbol is a circled letter for globally available toggles and a parenthesized
...
** Search char with f, F, t and T and repeart search char with , and ;
The ~​,​~ and ~;~ keys don't repeat the last ~f~, ~t~, ~F~, or ~T~ commands in
Spacemacs because these keys are taken by the major mode leader key and the
project leader key respectively.

To compensate for this loss of functionality, Spacemacs comes with =evil-snipe=
plugin which supercharges the keys ~f~, ~F~, ~t~ and ~T~ so they can be repeated
just after a search to behave like ~,~ and ~;~. For instance ~f n f f~ will go
to the third occurrence of the =n= character.

Once the search for a char is over, the only way to search for this char again
is to press ~f~, ~F~, ~t~ or ~T~ again. This is a minor inconvenience but in
the end it is only one more character to type.
...
| [[https://github.com/juanjux/evil-search-highlight-persist][evil-snipe                   ]] | replace search char behavior (~f~, ~F~, ~t~ and ~T~ |
...
To open this file use ~SPC V e d~. You can edit it to change or add/remove
environment variables. Use ~SPC V e R~ to reload it.
...
with ~SPC V e C~. Note that this action will overwrite =~/.spacemacs.env=.
...
layer. For more information on leader keys see the [[#leader-keys][Concepts documentation]].
...
** Searching
*** Searching for a character in the buffer
Spacemacs comes with =evil-snipe= preinstalled which supercharges the ~f~, ~F~,
~t~ and ~T~ commands and add ~s~ and ~S~ to search for two characters instead of
one.

Key bindings in normal state:
...
**** Improved f, F, t and T search behavior
With =evil-snipe= ~f~, ~F~, ~t~ and ~T~ search for a single character in the
current line (by default). Immediatly after a search the keys ~f~, ~F~, ~t~ and
~T~ can be repeated to continue searching for the next or previous characters.

**** Two-characters search with s and S
With the ~s~ and ~S~ keys it is possible to do a simple search like ~f~ and ~t~
but for two characters. This makes the search a lot more precise than regular
~f~ and ~t~ searches.

**** Available search scopes
=evil-snipe= also adds several scope options for searches. Set =evil-snipe-scope=
and =evil-snipe-repeat-scope= to one of these (the default value is =whole-line=):
